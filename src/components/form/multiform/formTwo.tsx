  ...
  const verifyCedula = async (cedula: string, type: "tutor" | "author") => {
    if (cedula.length === 8) { 
      try {
        ...
        const response = await axios.get(`http://10.200.36.15:3000/personas?identificacion=${cedula}`, {
          headers: {
            ...
            return (
              <div key={index} className="flex flex-col gap-3">
                <input
                  ...
                  onChange={(e) => handleTutorCedulaChange(index, e.target.value)}
                />
            ...
            return (
              <div key={index} className="flex flex-col gap-3">
                <input
                  ...
                  placeholder="Cédula"
                  className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4"
                  onChange={(e) => handleAuthorCedulaChange(index, e.target.value)}
                />
                {cedula && cedula.length === 8 && (
                  <div className="flex items-center gap-2">
                  {verification ? (
